...
%     [reduction, umap, clusterIdentifiers, extras]=run_umap(ds_sm_cellFR, 'cluster_output','graphic','cluster_detail','very low');
     [dch_idx, time_idx, dchTimeSec,dchStartDelaySec, smth_clusterIdentifiers,Fs, trialClust] = identifyUmapDecoherenceTimeBand(clusterIdentifiers, cells, ds_factor);
    idxClusterArray{i} = smth_clusterIdentifiers;
...
highlightedDecoherenceIndx = cell(numel(seshes),1);
for i = 1:numel(seshes)
    iCA = idxClusterArray{i};
    iDI = decoherenceTimeIdx{i};
    highlightedDecoherenceIndx{i} = iCA;
    highlightedDecoherenceIndx{i}(min(iDI):max(iDI)) = 30;
end
...
colormap jet
yt = get(gca, 'YTick'); 
set(gca, 'YTick',yt, 'YTickLabel',round(yt*Fs/60))
ylabel('Minutes');
