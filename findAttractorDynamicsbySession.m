...
pearson_rhoMatrix = nan(numel(seshes),12);
...
for i = 1:numel(seshes)
    ...
    save_figs = true;
    try
        [~,pearson_rho] = findAttractorDynamics(seshCells,save_figs,sf);
%         slopeMatrix(i,:) = slope;
...
time_only_rhoMatrix = pearson_rhoMatrix(:,5:8);
...
% x = vertcat(...
%     repmat({'baseline vs cntrl'}, rhoMatrixSize,1),...
%     repmat({'baseline vs acuteKet'}, rhoMatrixSize,1),...
%     repmat({'cntrl vs lateKet'}, rhoMatrixSize,1), ...
%     repmat({'lateKet vs gainChange'}, rhoMatrixSize,1),...
x = vertcat(...
	repmat({'baseline(time) vs cntrl(time)'}, rhoMatrixSize,1),...
	repmat({'baseline(time) vs acuteKet(time)'}, rhoMatrixSize,1),...
	repmat({'cntrl(time) vs lateKet(time)'}, rhoMatrixSize,1),...
	repmat({'lateKet(time) vs gainChange(time)'}, rhoMatrixSize,1)...
    );
% 	repmat({'spatialBaseline vs timebinnedBaseline'}, rhoMatrixSize,1),...
% 	repmat({'acuteKet(spatial) vs acuteKet(time)'}, rhoMatrixSize,1),...
% 	repmat({'control(spatial) vs acuteKet(time)'}, rhoMatrixSize,1),...
% 	repmat({'gainChange(spatial) vs gainChange(time)'}, rhoMatrixSize,1)...
%     );


% rhoMatrixSize = size(linearizedPearson_rhoMatrix,1);
% x = vertcat(repmat({'baseline vs cntrl'}, rhoMatrixSize,1),...
%     repmat({'baseline vs acuteKet'}, rhoMatrixSize,1),...
%     repmat({'cntrl vs lateKet'}, rhoMatrixSize,1), ...
%     repmat({'lateKet vs gainChange'}, rhoMatrixSize,1));
% 
...
g.set_color_options('chroma',0);
g.set_title('Pearson Correlations between Stable Cell Paris');
g.set_names('x','','y', 'Rho');
% x={'baselineA vs baselineB','baseline vs cntrl','baseline vs acuteKet','cntrl vs acuteKet','cntrl vs lateKet','baseline vs acuteKet','acuteKet vs lateKet','lateKet vs gainChange'};
g.set_order_options('x',0);
g.draw();
