...
% 
ketamineIndx = dataset.sp.st>ketamineInjxTime;
valid_idx(ketamineIndx) = 0;
%%
bins = 0:40:3840;
...
%%
figure();
pwelch(mean(spikeMat,1)',[],[],[],1/time_bins,'onesided','psd');

% figure(3);
% [pxx, freq] = pwelch(mean(spikeMat,1)',[],[],[],1/time_bins);
% plot(freq,10*(pxx),'k','linewidth',1.2);
% xlabel('Frequency (Hz)'); 
% ylabel('Spectral Density (Hz^{-1})');
% % periodogram(mean(spikeMat,1)',[],'onesided',[],1/time_bins)
