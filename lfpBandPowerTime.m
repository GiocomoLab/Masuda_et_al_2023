function [lfpByChannel, allPowerEst, F, allPowerVar] = lfpBandPowerTime(lfpFilename, lfpFs, nChansInFile, freqBand)
...
sampStarts = round(linspace(lfpFs*60*1, nSamps, nClips+1)); % skip first 10 secs
nClipSamps = round(lfpFs*clipDur);

mmf = memmapfile(lfpFilename, 'Format', {'int16', [nChansInFile nSamps], 'x'});

% dsSamps = downsample(1:nSamps, 50);
...
%     
%     thisDat = double(mmf.Data.x(:, dsSamps));
