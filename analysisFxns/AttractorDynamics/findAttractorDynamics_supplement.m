function [slope,pearson_rho] = findAttractorDynamics_supplement(cells,save_figs,sf)
...
if(sum(goodPairsIndx)>5)
    ...
    set(h1,'Position',[100 100 1500 600]);
%%%% 
    ...
    scatter(X,Y,markerSize,'MarkerFaceColor','k','MarkerFaceAlpha',markerFaceAlpha,'MarkerEdgeColor','w','LineWidth',0.1);
    axis(axisSize)
    title(sprintf("baseline vs gainChange: rho %.3f",rho));
    hl = lsline;
    B = [ones(size(hl.XData(:))), hl.XData(:)]\hl.YData(:);
    slope(tileNum) = B(2);
    pearson_rho(tileNum) = rho;
    ...
    slope(tileNum) = B(2);
    rho = corr(X,Y,'rows', 'complete');
    title(sprintf("cntrl vs acuteKet: rho %.3f",rho));
    pearson_rho(tileNum) = rho;
    ...
    scatter(X,Y,markerSize,'MarkerFaceColor','k','MarkerFaceAlpha',markerFaceAlpha,'MarkerEdgeColor','w','LineWidth',0.1);
    axis(axisSize)
    title(sprintf("cntrl vs gainChange: rho %.3f",rho));
    hl = lsline;
    B = [ones(size(hl.XData(:))), hl.XData(:)]\hl.YData(:);
    slope(tileNum) = B(2);
    pearson_rho(tileNum) = rho;
    ...
    nexttile
    tileNum = 8;
    X = timebinned_rho3t(timebinned_significantCellPairs);
    Y = timebinned_rho5t(timebinned_significantCellPairs);
    rho = corr(X,Y,'rows', 'complete');
    scatter(X,Y,markerSize,'MarkerFaceColor','k','MarkerFaceAlpha',markerFaceAlpha,'MarkerEdgeColor','w','LineWidth',0.1);
    axis(axisSize)
    title(sprintf("acuteKet vs lateKet: rho %.3f",rho));
    hl = lsline;
    B = [ones(size(hl.XData(:))), hl.XData(:)]\hl.YData(:);
    slope(tileNum) = B(2);
    pearson_rho(tileNum) = rho;
    ...
    scatter(X,Y,markerSize,'MarkerFaceColor','k','MarkerFaceAlpha',markerFaceAlpha,'MarkerEdgeColor','w','LineWidth',0.1);
    axis(axisSize)
    title(sprintf("acuteKet vs gainChange: rho %.3f",rho));
    hl = lsline;
    B = [ones(size(hl.XData(:))), hl.XData(:)]\hl.YData(:);
    slope(tileNum) = B(2);
    pearson_rho(tileNum) = rho;
    ...
    if save_figs
        saveas(h1,fullfile(sf.image_save_dir,sprintf('%s%s%s%s%d.svg',sf.name,'_',sf.sessionDate,'_sesh',sf.seshNum)),'svg');
    end
end
