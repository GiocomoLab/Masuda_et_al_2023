function plot_AttractorDynamicsbySession_supplement(cells,save_figs,folderName)
% folderName = "wt_cell_gainchange_only";
folderName = "time_wt_cell_stable_Supplement";

close all;
seshes = unique(cellfun(@num2str,cells.metadata(:,1),'uni',0));
% slopeMatrix = nan(numel(seshes),4);
pearson_rhoMatrix = nan(numel(seshes),10);
...
for i = 1:numel(seshes)
    ...
    save_figs = true;
    try
        [~,pearson_rho] = findAttractorDynamics_supplement(seshCells,save_figs,sf);
%         slopeMatrix(i,:) = slope;
...
time_only_rhoMatrix = pearson_rhoMatrix(:,1:10);
...
x = vertcat(...
	repmat({'baseline vs control'}, rhoMatrixSize,1),...
	repmat({'baseline vs acuteKet'}, rhoMatrixSize,1),...
	repmat({'baseline vs lateKet'}, rhoMatrixSize,1),...
    repmat({'baseline vs gainChange'}, rhoMatrixSize,1),...
    repmat({'control vs acuteKet'}, rhoMatrixSize,1),...
	repmat({'control vs lateKet'}, rhoMatrixSize,1),...
    repmat({'control vs gainChange'}, rhoMatrixSize,1),...
    repmat({'acuteKet vs lateKet'}, rhoMatrixSize,1),...
    repmat({'acuteKet vs gainChange'}, rhoMatrixSize,1),...
